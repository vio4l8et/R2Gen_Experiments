...
class R2GenLlama(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        self.llama_dtype = self.llama.model.embed_tokens.weight.dtype

        d_vis = args.d_vf
        d_llm = self.llama.config.hidden_size
        self.visual_proj = nn.Linear(d_vis, d_llm)

        self.num_query_tokens = getattr(args, "num_query_tokens", 32)
        self.query_tokens = nn.Parameter(
            torch.randn(self.num_query_tokens, d_llm)
        )
        self.qformer = nn.MultiheadAttention(
            embed_dim=d_llm,
            num_heads=getattr(args, "qformer_num_heads", 8),
            batch_first=True,
        )
    ...
    def encode_image(self, images):
        ...
        patch_feats = self.visual_proj(patch_feats)
        avg_feats = self.visual_proj(avg_feats).unsqueeze(1)
        feats = torch.cat([avg_feats, patch_feats], dim=1)
        B, S, D = feats.size()
        queries = self.query_tokens.unsqueeze(0).expand(B, -1, -1)
        visual_emb, _ = self.qformer(queries, feats, feats)
